# github.md
`design-scrapbook`のセットアップ時に学んだ、GitHub運用の基礎知識まとめ。
...
- スコープに **repo** をチェック
- 発行された `ghp_...` の文字列をコピーして保管（再表示不可）
- clone/push時、Password欄に**パスワードの代わりにこのトークンを入力**
...
| `design-scrapbook`のような継続的に育てるナレッジ | ◎ 作る |
| プロジェクトを横断する共有リソース（`design-ops`等） | ◎ 作る |
...
- `git --version` でGit自体が使えるか確認
- `git remote -v` で接続先URLが正しいか確認
- `git status` で今の変更状況（add/commit/push漏れがないか）確認
- エラーメッセージが出たら、**その場でコピーしてそのまま検索・相談する**（推測で対処しない）
- ターミナルの出力結果をそのままコピペで再実行してしまうと、意図しないコマンドとして実行されエラーになることがあるので注意


## ゼロから始める場合の全体フロー（新規アカウント想定）
...
- `https://github.com` にアクセスし、**Sign up** からアカウント作成
- メールアドレス・ユーザー名・パスワードを設定
- 確認メールのリンクをクリックして本登録完了
...
### Step 3：認証方法を用意する
前述の「GitHub認証の2つの方法」を参照し、PATかSSH鍵のどちらかを
事前に用意しておく（この後の全操作で必要になる）。


## リポジトリ作成〜ローカル連携（2つのやり方）
...
- Repository name を入力（例：`my-new-project`）
- Public / Private を選択
- **Add a README file** にチェックを入れておく（空リポジトリを避けられる）
...
**ローカルへの連携（VS Codeの場合）**
- VS Codeを開き、`Cmd+Shift+P` でコマンドパレットを開く
- **Git: Clone** と入力して選択
- GitHubのリポジトリURLを貼り付け、またはGitHubアカウント連携済みなら
  一覧から選択
- 保存先フォルダを選ぶ
- 「開きますか？」と聞かれたら **Open** を選択
...
**VS Codeの場合**
- 作業したいフォルダをVS Codeで開く（**File → Open Folder**）
- 左サイドバーのソース管理アイコン（分岐マークのアイコン）をクリック
- **Initialize Repository** ボタンをクリック
- ファイルを変更後、変更一覧の「+」でステージに追加
- 上部のメッセージ欄にコミット内容を入力し、**Commit**
- **Publish Branch** ボタンをクリック → GitHub上に新規リポジトリとして
  公開するか聞かれるので、リポジトリ名を確認して **Publish**


## 複数リポジトリを使い分けてコミット・プッシュする方法
...
**コミット・プッシュの操作自体は共通**
- 左サイドバーのソース管理アイコンを開く
- 変更されたファイルが一覧表示される（今、開いているフォルダ＝リポジトリの変更のみ表示される）
- 「+」でステージに追加（全部まとめてでも、1ファイルずつでも可）
- 上部にコミットメッセージを入力し、`Cmd+Enter`でコミット
- 下部または左下の同期ボタン（雲のアイコンや「Sync Changes」）をクリックしてプッシュ
