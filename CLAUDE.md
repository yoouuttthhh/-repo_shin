...
- 「なぜ重要か」の理由がない提案はしない
- mdファイルに新しいスタイルルール（見出し・箇条書き・テーブルなどの書式ルール）を追加・変更した場合は、指示がなくても同時に `Template/template-style.md` にも反映する
- ファイル・フォルダー構成に変更（追加・削除・リネーム）があった場合は、指示がなくても同時に `README.md` と `CLAUDE.md` の「ファイル構成」を更新する
- リポジトリ内で何らかの変更（ファイル追加・削除・内容変更）があった場合は、指示がなくても `CHANGELOG.md` に変更記録を残す（書き方は [CHANGELOG-tem.md](./Template/CHANGELOG-tem.md) の「レポジトリーの例」に従う）
